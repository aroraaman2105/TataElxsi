...
import { getNavSections } from '../config/routes';
...
export default function Navbar({ onMenuClick }) {
  ...
  const navItems = [...mainNav, ...sharedNav];
  ...
  return (
    <motion.header
      ...
      className="h-16 lg:h-20 border-b border-[var(--app-border)] bg-[var(--app-nav-bg)] backdrop-blur-xl flex items-center justify-between px-4 sm:px-6 flex-shrink-0 sticky top-0 z-30"
    >
      <div className="flex items-center gap-4 min-w-0 h-full">
        ...
        {personaConfig && (
          <div
            className="hidden md:flex items-center gap-2 px-2.5 py-1 rounded-lg border text-xs font-medium"
            style={{
          ...
          >
            <span className="w-1.5 h-1.5 rounded-full" style={{ background: personaConfig.accent }} />
            {personaConfig.shortLabel}
          </div>
        )}
      </div>

      <nav className="hidden lg:flex items-center gap-1 h-full mx-4 overflow-x-auto">
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`relative px-3 h-full flex items-center text-sm font-medium rounded-lg whitespace-nowrap ${
      ...
      <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
        ...
        <motion.div
          className="w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center font-semibold text-sm border"
          style={
            personaConfig
              ? {
                  color: personaConfig.accent,
